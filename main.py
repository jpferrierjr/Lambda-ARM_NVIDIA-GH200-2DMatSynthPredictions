...
base_path           = os.path.join( '/home/ubuntu', 'GH200' )           # This is my custom mounted storage on the server. I'd usually use `os.getcwd()`
save_path           = os.path.join( base_path, "DFT Output DFTD4" )
mol_path            = os.path.join( save_path, "molecules" )
...
PS_directory        = os.path.join( current_directory, "Pubchem Structures" )
PS_Files            = [ os.path.join(PS_directory, f) for f in os.listdir( PS_directory ) if os.path.isfile( os.path.join( PS_directory, f ) ) ]
...
#region functions
## General
# Calculates grid spacing for plane wave cutoff
# This is just an estimate to reduce computation
def calculate_grid_spacing( cutoff_ev:float, functional_type:str = 'PBE', limit:int = 0 )->float:

    assert isinstance(limit, int) and limit<3, "Limit can only be an integer between 0 and 2"

    
    # Empirical formula based on GPAW recommendations and practical experience
    # For GGA: h ≈ 0.6/sqrt(E_cut_eV/100) gives reasonable values
    # This gives h ≈ 0.2 for ~400 eV with GGA, which matches common practice
    h_base = 0.6/np.sqrt( cutoff_ev/100. )
    
    # Adjustment factors for different functional types
    # (smaller factor = finer grid needed)
    adjustment_factors = {
        'PBE-D4': 1.0,
        'r2SCAND4': 0.75,
    }
    ...
    h_recommended   = h_base*adjustment_factors[functional_type]
    h_conservative  = h_recommended * 0.8    # 20% finer than recommended
    h_aggressive    = h_recommended * 1.2    # 20% coarser than recommended
    
    # Ensure minimum grid spacing doesn't go below practical limits
    h_min = 0.05  # Minimum practical grid spacing
    h_max = 0.25  # Maximum reasonable grid spacing
    
    h_recommended   = max( h_min, min( h_max, h_recommended ) )
    h_conservative  = max( h_min, min( h_max, h_conservative ) )
    h_aggressive    = max( h_min, min( h_max, h_aggressive ) )
    
    # Round to reasonable precision
    h_recommended   = round( h_recommended, 3 )
    h_conservative  = round( h_conservative, 3 )
    h_aggressive    = round( h_aggressive, 3 )

    hs              = [ h_conservative, h_recommended, h_aggressive ]
    
    return hs[limit]

# Returns the proper GPAW calculator
def get_calculator( 
        xc:str                      = 'PBE',
        encut:float                 = 500.,
        hund:bool                   = False, 
    ...
    if is_crystal:
        ...
        h = calculate_grid_spacing( encut, xc )

        # For this calculator we'll be using dzp
        calc =  GPAW(   mode        = PW(encut),
                        basis       = 'dzp',
                        xc          = xc_list[xc],
                        hund        = hund,
                        h           = h,
                        occupations = { 'name': 'fermi-dirac','width': kB*300. },
                        convergence = convergence,
                        symmetry    = symm,
                        parallel    = p_param )
        
    if custom_params is not None:
        calc = GPAW( **custom_params )
    
    return calc

# Checks normalized changes in X,Y
...
# Optimizes the molecule calculators
def molecular_optimizer( 
        molecule:Atoms, 
        molecule_path:str   = "" ):
    ...
    # Check Hund
    hund_rule   = len(molecule.get_atomic_numbers())==1
    ...
    for xc in xc_list:
        ...
        encut_file      = os.path.join( molecule_path, f"XC-{xc_name}_encut.npy" )
        vac_file        = os.path.join( molecule_path, f"XC-{xc_name}_vacuum.npy" )
        ...
        if encut_exists:
            ...
            # Do cutoff energy
            ENS         = np.linspace( 100., 800., 20 )
            pot_ens     = []
            val_list    = []

            for i, E in enumerate( ENS ):
                molecule.set_calculator( get_calculator( xc = xc, encut = E, hund = hund_rule ) )
                    
                pot_ens.append( molecule.get_potential_energy() )
                val_list.append( E )
                if i > 0:
                    if check_normalized_slope( val_list, pot_ens, minsteps = 4 ):
                        break
        ...
        if vac_exists:
            ...
            for i, V in enumerate( VAC ):
                ...
                molecule.set_calculator( get_calculator( xc = xc, encut = return_vals[xc]['cutoff_energy'], hund = hund_rule ) )

                molecule.center( vacuum = V )

                pot_ens.append( molecule.get_potential_energy() )
                val_list.append( V )
                if i > 0:
                    if check_normalized_slope( val_list, pot_ens ):
                        break
...
# Optimizes the crystal calculators
...
for file in PS_Files:
    ...
    data_base       = data['PC_Compounds'][0]
    elements        = data_base['atoms']['element']
    charge          = data_base['charge']

    # Get the coordinates of each atom
    x_coords        = data_base['coords'][0]['conformers'][0]['x']
    y_coords        = data_base['coords'][0]['conformers'][0]['y']
    
    # Since all files are in 2D, set z = 0.
    # This will rectify in relaxation.
    z_coords        = [ 0. for _ in range( len( x_coords ) )]
    positions       = [ list(pos) for pos in zip(x_coords, y_coords, z_coords)]

    # Get the filename
    fn              = os.path.basename(file)

    # Remove the last 5 values (.json)
    fn              = fn[:-5]

    # Build and save the molecule to the dictionary
    molecules[fn]   = Atoms( positions = positions, numbers = elements )
#endregion

#region Optimize molecule GPAW settings
molecule_calc_vals  = {}

# Cycle through each molecule and optimize
...
#region Optimize molecule structures
# Cycle through each molecule and optimize
for molecule in molecules:
    ...
    # Create Trajectories path
    molecule_traj = os.path.join( molecule_path, "trajectories" )
    os.makedirs( molecule_traj, exist_ok = True )

    # Cycle through XCs
    for xc in xc_list:
        
        # Check if relaxed.gpaw already exists
        relaxed_poten   = os.path.join( molecule_path, f'relaxed_{xc}.npy' )

        if not os.path.isfile( relaxed_poten ):

            # Better setup when running all cells in order.
            atoms       = molecules[molecule].copy()
            xc_opt_vals = molecule_calc_vals[molecule][xc]
            
            # Explicitly build the calculator or ASE gets mad.
            encut       = xc_opt_vals['cutoff_energy']
            vacuum      = xc_opt_vals['vacuum']
            hund        = ( len(atoms)==1 )

            gcalc       = get_calculator( xc = xc, encut = encut, hund = hund )

            # Build the trajectory file name
            traj_file   = os.path.join( molecule_traj, f'relaxation_{xc}.traj' )

            if xc == "PBE-D4":
                # Perturb the atoms slightly. (randomly between +/- 0.05 angstroms)
                ...
                PBE_traj    = os.path.join( molecule_traj, f'relaxation_PBE-D4.traj' )
                if os.path.isfile( PBE_traj ):
                    atoms   = read( PBE_traj, index = '-1' )

            # Build the DFTD4 correction
            calc        = DFTD4( method = dftd4_method[xc] ).add_calculator( gcalc )
            atoms.set_calculator( calc )

            # Relax Structure
            dyn = FIRE( atoms = atoms, trajectory = traj_file )
            dyn.run( fmax = fmax )
...
for molecule in molecules:
    ...
    for xc in xc_list:
        ...
        vib_file    = os.path.join( molecule_vibs, f"vibrationalData_{xc}.npy" )

        if not os.path.isfile( vib_file ):

            rel_vibs    = os.path.join( molecule_vibs, f"vibrations_{xc}" )
            vib_gpw     = os.path.join( molecule_vibs, f'vibrations_{xc}.gpw' )
            mol         = read( os.path.join( molecule_traj, f'relaxation_{xc}.traj'), index="-1" ) # Just grab the last one

            # Build the calc
            encut       = molecule_calc_vals[molecule][xc]['cutoff_energy']
            vacuum      = molecule_calc_vals[molecule][xc]['vacuum']
            hund        = ( len(mol)==1 )
            ...
            vib         = Vibrations( mol, name = rel_vibs )
            vib.run()
...
for bulk, qmat in combinations:
    ...
    for xc in xc_list:
        ...
        if os.path.isfile( het_bind_en ):
            ...
            optimizer = FIRE( het, trajectory = het_traj_file, append_trajectory = True, dt = 0.1 )
            optimizer.run( fmax = fmax )
...
for qmat in materials_2d:
    crys_path   = os.path.join( qm_path, f"{qmat}" )
    
    if not qmat in qmat_ph:
        qmat_ph[qmat] = {}

    for xc in xc_list:
        if not xc in qmat_ph[qmat]:
            qmat_ph[qmat][xc] = {}

        size            = 6 # Phonon supercell size
        vib_path        = os.path.join( crys_path, f'vib_{xc}-{size}x{size}x1' )
        os.makedirs( vib_path, exist_ok = True )

        vib_en_file     = os.path.join( vib_path, 'ph_energies.npy' )
        vib_DOS_file    = os.path.join( vib_path, 'ph_DOS.npy' )

        if os.path.isfile( vib_en_file ) and os.path.isfile( vib_DOS_file ):
            # Just open these
            qmat_ph[qmat][xc]['energies']   = np.load( vib_en_file )
            qmat_ph[qmat][xc]['DOS']        = np.load( vib_DOS_file )

        # Else, compute them
        else:

            # Copy 2D material
            mat         = materials_2d[qmat].copy()
            ...
            # Build calculator
            gpw_calc    = get_calculator( xc = xc, encut = qm_calc_vals[qmat][xc]['cutoff_energy'], is_crystal = True )
            calc        = DFTD4( method = dftd4_method[xc] ).add_calculator( gpw_calc )

            mat.set_calculator( calc )

            # Relax the 2D material
            optimizer = FIRE( mat, trajectory = rlx_traj, append_trajectory = True )
            optimizer.run( fmax = fmax )
            ...
            qmat_ph[qmat][xc]['energies']   = dos.get_energies()
